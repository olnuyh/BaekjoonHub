...
public class Main {
    ...
    public static void move() {
        ...
        while (!pq.isEmpty()) {
            ...
            for (int i = 0; i < 4; i++) {
                int nr = room.r + deltas[i][0];
                int nc = room.c + deltas[i][1];

                if (nr < 0 || nr >= N || nc < 0 || nc >= M || result[nr][nc] != Integer.MAX_VALUE) {
                    continue;
                }

                if (result[nr][nc] > maze[nr][nc] + room.wall) {
                    result[nr][nc] = maze[nr][nc] + room.wall;
                    pq.add(new Room(nr, nc, result[nr][nc]));
                }
            }
        }
    }
